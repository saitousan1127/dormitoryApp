...
class PrememberModel extends BaseModel
{
    ...
    public function delete_premember_and_regist_member($userdata)
    {
        try {
            ...
            $stmh->execute();
            $sql = "INSERT  INTO member (username, password, last_name, first_name, h_last_name, h_first_name, k_last_name, k_first_name, class, roomnum, birthday, reg_date )
            VALUES ( :username, :password, :last_name, :first_name, :h_last_name, :h_first_name, :k_last_name, :k_first_name, :class, :roomnum, :birthday , now() )";
            $stmh = $this->pdo->prepare($sql);
            $stmh->bindValue(':username', $userdata['username'], PDO::PARAM_STR);
            $stmh->bindValue(':password', $userdata['password'], PDO::PARAM_STR);
            $stmh->bindValue(':last_name', $userdata['last_name'], PDO::PARAM_STR);
